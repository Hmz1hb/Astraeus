...
<div class="modal fade" id="errorModal" tabindex="-1" aria-labelledby="errorModalLabel" aria-hidden="true">
    ...
    <section class="vh-100 bg-image" style="background-image: url('./img/2.jpg');">
...
$(document).on('submit', '#resetPasswordForm', function(e) {
  ...
  var confirmPassword = $('#form3Example5cg').val();

  // Check if the password and confirm password match
  if (password !== confirmPassword) {
    $('#messageModalBody').text('Password and Confirm Password do not match');
  ...
  console.log('Sending AJAX request to reset password');

  $.ajax({
    url: 'resetPasswordconfirmation.php',
    type: 'POST',
    ...
    error: function() {
        $('#messageModalBody').text('An error occurred during password reset');
      $('#messageModal').modal('show');
    }
  });
});
